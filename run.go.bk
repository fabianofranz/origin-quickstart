...
func main() {
	...
	etcd := fmt.Sprintf("%s:%s", host, etcdPort)
	...
	cfg := &config{
		...
		EtcdAddr:       flagtypes.Addr{Value: etcd, DefaultScheme: "http", DefaultPort: etcdPort}.Default(),
		KubernetesAddr: flagtypes.Addr{DefaultScheme: "http", DefaultPort: port}.Default(),
